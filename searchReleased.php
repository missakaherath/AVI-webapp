            ...
            <div>
                <table class="container-table">
                    ...
                    if ($_SESSION['set'] = true) {
                        $searchedReleasedVehicles = $_SESSION['searchedReleasedVehicles'];
                        if (sizeof($searchedReleasedVehicles) == 0) {
                            echo "<h4 style='margin-left : 50px'; font-color : #002d24;>No Results Were Found.</h4>";
                        }
                        foreach ($searchedReleasedVehicles as $searchedReleased) {
                            $vehicleNumber = $searchedReleased['vehicle_number'];

                            $releasedDate = $searchedReleased['releasedDatetime'];
                            $datetime = new DateTime($releasedDate);
                            $date = $datetime->format('Y-m-d');

                            $releasedTime = $searchedReleased['releasedDatetime'];
